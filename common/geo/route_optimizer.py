...
class RoutePoint:
    ...
    must_arrive_by: time | None = None  # 訂位等硬性時間窗（只有上限，沒有「太早到」的下限）；
                                         # None 代表這個點沒有時間限制，維持自由排序
...
# ============================================================================
# 時間窗（訂位時間）：只有少數景點會被使用者標「必須在幾點前到」，其餘景點維持
# 自由排序——不是每個點都上時間窗，那是完全不同量級的問題（VRPTW），這裡刻意
# 縮小範圍成「檢查點」：先用 optimize_route() 算出忽略時間窗的最佳順序（多數情況下
# 地理最佳路徑本身就跟時間先後一致，不會跟時間窗衝突），只有真的衝突時才對違規的
# 點做局部修復（往前搬到更早的位置），不是一開始就用完整的時間窗排班演算法硬解。
# ============================================================================


def _minutes_since_midnight(t: time) -> float:
    return t.hour * 60 + t.minute


def _time_from_minutes(minutes: float) -> time:
    total = int(round(minutes)) % (24 * 60)  # 理論上一天的行程不會跨過午夜，防呆用
    return time(hour=total // 60, minute=total % 60)


def _legs_from_sequence(
    sequence: list[RoutePoint],
    ...
    for a, b in zip(sequence, sequence[1:]):
        i, j = id_to_index[a.id], id_to_index[b.id]
        key = (i, j) if i < j else (j, i)
        detail = details[key]
        legs.append(Leg(from_id=a.id, to_id=b.id, minutes=detail["minutes"], via=detail["via"], detail=detail["detail"]))
    return legs


def _compute_etas(sequence: list[RoutePoint], legs: list[Leg], day_start: time) -> dict[str, time]:
    """算出 sequence 裡每一站的實際時鐘到達時間（到達當下，還沒加上該站的停留時間）。"""
    etas = {sequence[0].id: day_start}
    elapsed = 0.0
    for point, leg in zip(sequence[1:], legs):
        elapsed += leg.minutes
        etas[point.id] = _time_from_minutes(_minutes_since_midnight(day_start) + elapsed)
        elapsed += point.suggested_stay_minutes or 0.0
    return etas


def _find_violations(sequence: list[RoutePoint], etas: dict[str, time]) -> list[dict]:
    violations = []
    for p in sequence:
        if p.must_arrive_by is None:
            continue
        eta_minutes = _minutes_since_midnight(etas[p.id])
        deadline_minutes = _minutes_since_midnight(p.must_arrive_by)
        if eta_minutes > deadline_minutes + 1e-6:
            violations.append({
                "id": p.id,
                "eta": etas[p.id],
                "deadline": p.must_arrive_by,
                "late_by_minutes": round(eta_minutes - deadline_minutes, 1),
            })
    return violations


def schedule_day_with_deadlines(
    points: list[RoutePoint],
    start_id: str,
    end_id: str | None,
    day_start: time,
) -> dict:
    """跟 optimize_route 介面類似，但 points 裡可能有幾個點帶 must_arrive_by。

    先照常算出忽略時間窗的最佳順序，再檢查每個有時間窗的點會不會遲到；遲到的話
    依時間窗先後（最早的先修）試著把該點搬到更早的位置，選「還能準時、且新增車程
    最少」的搬法；搬到最前面（緊接出發點之後）還是來不及，就是這天真的排不下，
    保留這個違規在回傳結果裡，不假裝排好。

    這是局部修復（只搬違規的點），不是重新對整條路徑跑一次完整最佳化，MVP 規模
    （每天最多 15 個景點）之下足夠用，複雜度也可控。

    回傳 {"order", "legs", "total_minutes", "etas": {id: time}, "violations": [...]}
    violations 是修復後仍然存在的違規（空list代表這天所有時間窗都滿足）。
    """
    base = optimize_route(points, start_id=start_id, end_id=end_id)
    order = list(base["order"])
    legs = list(base["legs"])

    matrix, details = build_distance_matrix(order)
    id_to_index = {p.id: i for i, p in enumerate(order)}

    etas = _compute_etas(order, legs, day_start)

    max_iterations = len(order) + 2  # 防呆上限，避免理論上不該發生的無窮迴圈
    for _ in range(max_iterations):
        violations = _find_violations(order, etas)
        if not violations:
            break

        target_violation = min(violations, key=lambda v: _minutes_since_midnight(v["deadline"]))
        idx = next(i for i, p in enumerate(order) if p.id == target_violation["id"])
        if idx == 0:
            break  # 起點本身不會有時間窗，防呆

        point = order[idx]
        best_order, best_legs, best_etas, best_added_cost = None, None, None, None
        current_total = sum(leg.minutes for leg in legs)

        for insert_at in range(1, idx + 1):
            candidate = order[:idx] + order[idx + 1:]
            candidate.insert(insert_at, point)
            candidate_legs = _legs_from_sequence(candidate, matrix, details, id_to_index)
            candidate_etas = _compute_etas(candidate, candidate_legs, day_start)

            eta_minutes = _minutes_since_midnight(candidate_etas[point.id])
            deadline_minutes = _minutes_since_midnight(point.must_arrive_by)
            if eta_minutes > deadline_minutes + 1e-6:
                continue

            added_cost = sum(leg.minutes for leg in candidate_legs) - current_total
            if best_added_cost is None or added_cost < best_added_cost:
                best_added_cost = added_cost
                best_order, best_legs, best_etas = candidate, candidate_legs, candidate_etas

        if best_order is None:
            break  # 搬到最前面都來不及，這個違規修不掉，停止修復、保留違規

        order, legs, etas = best_order, best_legs, best_etas
...
def chain_optimize_days(
    ...
    day_start_times: list[time] | None = None,
) -> dict:
    ...
    預設重用 optimize_route()（NN + 2-opt），但該天只要有任何景點帶 must_arrive_by
    （訂位時間），就改用 schedule_day_with_deadlines() 排（day_start_times 對應
    那天的出發時鐘時間，沒給就預設 09:00）。
    ...
    回傳 {"days": [[RoutePoint,...], ...], "legs": [Leg,...], "total_minutes": float,
          "etas": {id: time}（只有走過 schedule_day_with_deadlines 的天才有值），
          "violations": [{"id", "eta", "deadline", "late_by_minutes"}, ...]（跨全部天攤平）}
    legs 是整趟行程攤平的相鄰站車程（含跨天銜接段），第一段從 start 出發算起。
    """
    result_days: list[list[RoutePoint]] = []
    all_legs: list[Leg] = []
    all_etas: dict[str, time] = {}
    ...
    for day_index, bucket in enumerate(day_buckets):
        ...
        has_deadline = any(p.must_arrive_by is not None for p in bucket)
        if has_deadline:
            day_start = (day_start_times[day_index] if day_start_times else None) or time(9, 0)
            day_result = schedule_day_with_deadlines(points, start_id=anchor_id, end_id=end_id, day_start=day_start)
            all_etas.update(day_result["etas"])
            all_violations.extend(day_result["violations"])
        else:
...
def evaluate_day_plan(
    ...
    調整「哪天先去」，每天內部的景點組合完全不變。day_start_times（如果有給）跟著
    day_buckets 一起重新排列，因為出發時間是跟著「這天的內容」走的，不是跟著天序
    位置走的（例如把第3天的內容換到第1天，訂位時間限制也要跟著搬過去）。

    回傳 {
      "chosen": {"days": [...], "legs": [...], "total_minutes": float, "etas": {...}, "violations": [...]},
      "suggested_order": list[int] | None,   # None 代表使用者選的天序已經是最好的
      "suggested": {...} | None,
      "minutes_saved": float,
    }
    """
    chosen = chain_optimize_days(day_buckets, start, end, day_start_times)
    ...
    reordered_start_times = [day_start_times[i] for i in candidate_order] if day_start_times else None
    suggested = chain_optimize_days(reordered_buckets, start, end, reordered_start_times)
