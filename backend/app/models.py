...
class DayBucket(BaseModel):
    ...
    attraction_ids: list[str] = Field(..., min_length=1, max_length=15)


class DayPlanRequest(BaseModel):
    """使用者自己分天選好景點，程式負責：1) 幫每天內部景點排最佳順序
    ...
    def _total_attraction_cap(self) -> "DayPlanRequest":
        total = sum(len(day.attraction_ids) for day in self.days)
        if total > 30:
            raise ValueError(f"景點總數（{total}）超過上限 30 筆")

        all_ids = [aid for day in self.days for aid in day.attraction_ids]
